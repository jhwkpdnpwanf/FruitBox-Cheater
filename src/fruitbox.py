...
def break_apple_right(array, row, col): # currnet row, current col
    ...
    for i in range(col + 1, COL):
        ...
        if hap == 10:
            ...
            return (1, row, col, tuple(partial_array)) # (break method, row, col, (value to recover))
    ...
    for i in range(row + 1, ROW):
        ...
        if hap == 10:
            for j in range(row, i + 1):
                array[j][col] = 0
            return (2, row, col, tuple(partial_array)) # (break method, row, col, (value to recover))
    return []
    ...
    while i < ROW:
        ...
        while j < COL:
            ...
            if hap == 10:
                ...
                return (3, row, col, tuple(map(tuple, partial_array)))  # (break method, row, col, (value to recover))
            j += 1
        i += 1
        ...
        for j in range(COL):
            ...
            if step_right:
                ...
                if search_best_path(array, trace_path):
                    return True
                method, row, col, value = trace_path[-1]
